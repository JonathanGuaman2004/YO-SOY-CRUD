...
type StoredEvent = Record<string, unknown> & { _table?: string; _eventDate?: string };
...
export class EventsService {
  ...
  async findAll(): Promise<object[]> {
    ...
    const merged: StoredEvent[] = [
      ...creates.map((e) => ({ ...e, _table: 'create_events', _eventDate: e.recorded_at })),
      ...updates.map((e) => ({ ...e, _table: 'update_events', _eventDate: e.timestamp })),
      ...deletes.map((e) => ({ ...e, _table: 'delete_events', _eventDate: e.createdAt })),
      ...queries.map((e) => ({ ...e, _table: 'query_events', _eventDate: e.event_date })),
    ];
  ...
  private normalizeAction(action?: string): string {
    ...
    if (!this.allowedActions.includes(normalized)) {
      throw new BadRequestException('Acción no permitida. Use CREATE, UPDATE, DELETE o QUERY.');
    }
    return normalized;
  }
  ...
  private validateDto(dto: CreateEventDto, action: string): void {
    ...
    if (requiredFields.some((field) => !this.clean(field))) {
      throw new BadRequestException('source, entity, action y title son obligatorios.');
    }

    if (this.clean(dto.source).length > 60) throw new BadRequestException('source no puede superar 60 caracteres.');
    if (this.clean(dto.entity).length > 60) throw new BadRequestException('entity no puede superar 60 caracteres.');
    if (this.clean(dto.title).length > 120) throw new BadRequestException('title no puede superar 120 caracteres.');
    if (this.clean(dto.description ?? '').length > 500) throw new BadRequestException('description no puede superar 500 caracteres.');

    if (action !== 'QUERY' && (!dto.payload || Object.keys(dto.payload).length === 0)) {
      throw new BadRequestException('payload es obligatorio para CREATE, UPDATE y DELETE.');
    }
  }
  ...
  private extractQueryTerm(payload: unknown): string {
    ...
    return this.clean(data.query ?? data.filtro ?? data.id ?? data.name ?? '').substring(0, 120);
  }
}
